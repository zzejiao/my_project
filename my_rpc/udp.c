...
struct socket init_socket(int port){
	...
	//create a UDP socket using a syscall socket()
	if ((my_socket.fd=socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1)//error check? if fails, die
	{//AF_INET meaning we use IPv4 protocols, SOCK_DGRAM for UDP
		die("socket");
	}
